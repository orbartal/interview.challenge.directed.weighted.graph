...
public class PathsByEdgesWeightAlgorithm {
	public int getNumberOfPaths(Graph graph, String start, String end, int min, int max) {
		...
		int[][] pathsCounter = new int[nodes.size()][max + 1];
		...
		for (int d = 0; d <= max; d++) {
			for (int n = 0; n < nodes.size(); n++) {
				...
				for (Entry<String, Edge> pair : edgesByTargetName.entrySet()) {
					...
					int weight = pair.getValue().getWeight();
					int dw = +d + weight;
					if (dw > max) {
						continue;
					}
					int targetIndex = nodeNameToIndex.get(targetName);
					pathsCounter[targetIndex][dw] += pathsCounter[n][d];
				}
			}
		}
